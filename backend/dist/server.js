...
app.post('/upload-mhtcet', upload.single('csvFile'), async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({
      message: 'MHT CET dataset updated successfully'
    });
  } catch (error) {
    console.error('Error updating MHT CET dataset:', error);
    res.status(500).json({
      error: 'Failed to update MHT CET dataset'
    });
  }
});
...
app.post('/upload-neet', upload.single('csvFile'), async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({
      message: 'NEET dataset updated successfully'
    });
  } catch (error) {
    console.error('Error updating NEET dataset:', error);
    res.status(500).json({
      error: 'Failed to update NEET dataset'
    });
  }
});
app.get('/download-mhtcet', async (req, res) => {
  try {
    ...
    res.status(500).json({
      error: 'Failed to download MHT CET dataset'
    });
  }
});
...
app.get('/download-neet', async (req, res) => {
  try {
    ...
    res.status(500).json({
      error: 'Failed to download NEET dataset'
    });
  }
});
...
app.post('/api/predict', async (req, res) => {
  ...
  try {
    const colleges = await collection.find(query).sort({
      percentile: -1
    }).toArray();
...
app.post('/api/Neetpredict', async (req, res) => {
  ...
  try {
    const colleges = await neetCollection.find(query).sort({
      max_rank: 1
    }).limit(50).toArray();
    res.json(colleges);
  } catch (error) {
...
app.get('/api/recommended', async (req, res) => {
  try {
    ...
    res.status(500).json({
      error: 'An error occurred while fetching recommended colleges'
    });
  }
});
...
app.get('/api/college-users', async (req, res) => {
  try {
    const collegeUsers = await usersCollection.find({
      type: "college"
    }).toArray();
    ...
    res.status(500).json({
      message: 'Error fetching college users and courses',
      error
    });
  }
});
app.get('/api/student-users', async (req, res) => {
  try {
    const studentUsers = await usersCollection.find({
      type: "student"
    }).toArray();
    const studentUserDetails = await Promise.all(studentUsers.map(async user => {
      const purchases = await boughtCoursesCollection.find({
        'user._id': user._id
      }).toArray();
      return {
        user,
    ...
    res.status(500).json({
      message: 'Error fetching student users and purchases',
      error
    });
  }
});
